...
#[test]
#[cfg_attr(target_os = "windows", ignore = "libm not available as .dll on Windows")]
fn test_extern_sqrt_basic() {
    ...
    match run_program(source) {
        Ok(Value::Number(n)) => {
            assert!((n - 4.0).abs() < 0.0001, "sqrt(16.0) should be 4.0, got {}", n);
        }
...
fn test_extern_pow_basic() {
    ...
    match run_program(source) {
        Ok(Value::Number(n)) => {
            assert!((n - 8.0).abs() < 0.0001, "pow(2.0, 3.0) should be 8.0, got {}", n);
        }
        Ok(other) => panic!("Expected number, got: {:?}", other),
        Err(e) => panic!("Program failed: {}", e),
    }
...
fn test_extern_multiple_calls() {
    ...
    match run_program(source) {
        Ok(Value::Number(n)) => {
            // 3.0 + 5.0 = 8.0
            assert!((n - 8.0).abs() < 0.0001, "sqrt(9) + sqrt(25) should be 8.0, got {}", n);
        }
        Ok(other) => panic!("Expected number, got: {:?}", other),
        Err(e) => panic!("Program failed: {}", e),
    }
...
fn test_extern_with_user_functions() {
    ...
    match run_program(source) {
        Ok(Value::Number(n)) => {
            assert!((n - 5.0).abs() < 0.0001, "hypotenuse(3, 4) should be 5.0, got {}", n);
        }
        Ok(other) => panic!("Expected number, got: {:?}", other),
        Err(e) => panic!("Program failed: {}", e),
    }
...
fn test_extern_ceil_floor() {
    ...
    match run_program(source) {
        Ok(Value::Number(n)) => {
            ...
            assert!((n - 7.0).abs() < 0.0001, "ceil(3.2) + floor(3.8) should be 7.0, got {}", n);
        }
        Ok(other) => panic!("Expected number, got: {:?}", other),
        Err(e) => panic!("Program failed: {}", e),
    }
...
fn test_extern_sin_cos() {
    ...
    match run_program(source) {
        Ok(Value::Number(n)) => {
            ...
            assert!((n - 1.0).abs() < 0.0001, "sin^2 + cos^2 should be 1.0, got {}", n);
        }
        Ok(other) => panic!("Expected number, got: {:?}", other),
        Err(e) => panic!("Program failed: {}", e),
    }
