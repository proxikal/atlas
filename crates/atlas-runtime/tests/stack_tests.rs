...
fn test_push_increases_size() {
    ...
    call_builtin("stackPush", &[stack.clone(), Value::Number(1.0)], dummy_span(), &security())
        .unwrap();
...
fn test_pop_lifo_order() {
    ...
    call_builtin("stackPush", &[stack.clone(), Value::Number(1.0)], dummy_span(), &security())
        .unwrap();
    call_builtin("stackPush", &[stack.clone(), Value::Number(2.0)], dummy_span(), &security())
        .unwrap();
    call_builtin("stackPush", &[stack.clone(), Value::Number(3.0)], dummy_span(), &security())
        .unwrap();
...
fn test_stack_accepts_any_value_type() {
    ...
    call_builtin("stackPush", &[stack.clone(), Value::Number(42.0)], dummy_span(), &security())
        .unwrap();
    call_builtin(
        "stackPush",
    ...
    .unwrap();
    call_builtin("stackPush", &[stack.clone(), Value::Bool(true)], dummy_span(), &security())
        .unwrap();
    call_builtin("stackPush", &[stack.clone(), Value::Null], dummy_span(), &security()).unwrap();
...
fn test_peek_doesnt_change_size() {
    ...
    let size_before = call_builtin("stackSize", &[stack.clone()], dummy_span(), &security()).unwrap();
...
fn test_multiple_stacks_are_independent() {
    ...
    call_builtin("stackPush", &[stack1.clone(), Value::Number(1.0)], dummy_span(), &security())
        .unwrap();
    call_builtin("stackPush", &[stack2.clone(), Value::Number(2.0)], dummy_span(), &security())
        .unwrap();
